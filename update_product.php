<?php
  ...
  if(isset($_POST["btn_update"])) {
    ...
    $sql_update = "update tbl_product set catename = N'".$cate_name."', name = N'".$product_name."', min_price = ".$product_minvalue.", max_price = ".$product_maxvalue.", description = ".$product_desc.", image = '".$image  ."', quantity = ".$product_quantity." where id =" .$cate_id;
              ...
              if(isset($_GET["task"]) && $_GET["task"]=="update") {
                 $id = $_GET["id"];
                  ...
                  if(mysqli_num_rows($result)>0) {
                     ...
                     while($row = mysqli_fetch_assoc($result)) {
                          echo "<input type='hidden' name='cate' value ='".$row["id"]."'>";
